...
const API_URL =
  (process.env.NEXT_PUBLIC_API_URL ||
    ...
    .replace(/\/$/, "") +
  "/api/user-details?pagination[page]=1&pagination[pageSize]=100";
...
export default function DashboardAnalytics() {
  ...
  const avgAge =
    users.length > 0
      ? Math.round(
          users.reduce((sum, u) => sum + (computeAge(u) || 0), 0) /
            users.length
        )
      : 0;
  ...
  const ageBuckets = { "18-25": 0, "26-35": 0, "36-50": 0, "51+": 0 };
  users.forEach((u) => {
  ...
  if (loading)
    return (
      <div className="flex w-full h-screen items-center justify-center 
        bg-white text-black dark:bg-neutral-950 dark:text-white transition-colors">
        Loading dashboard...
      </div>
    );
    ...
    <div className="min-h-screen 
      ...
      <div className="flex flex-col items-center mb-10 pt-8">
        <span className="inline-flex items-center px-4 py-1.5 rounded-full text-sm 
          ...
          shadow font-semibold mb-4 transition-colors">
        ...
        <p className="text-lg max-w-2xl text-center mt-3 
          text-neutral-700 dark:text-neutral-300 transition-colors">
          Comprehensive insights and statistics about your user management system.
        </p>
      </div>

      {/* Stat Cards */}
      <div className="grid gap-5 grid-cols-1 md:grid-cols-2 lg:grid-cols-4 mb-10 max-w-7xl mx-auto">
        <div className="bg-neutral-100 dark:bg-neutral-900 rounded-xl shadow p-6 flex flex-col transition-colors">
          <div className="flex items-center justify-between mb-2">
            <span className="text-lg font-semibold">Total Users</span>
            <UserGroupIcon className="w-8 h-8 text-gray-600 dark:text-gray-300" />
          </div>
          ...
          <span className="text-sm flex items-center">
            ...
            <span className="text-green-600 dark:text-green-400">{activeUsers} Active</span>
          </span>
        </div>
        ...
        <div className="bg-neutral-100 dark:bg-neutral-900 rounded-xl shadow p-6 flex flex-col transition-colors">
          <div className="flex items-center justify-between mb-2">
            ...
            <ArrowTrendingUpIcon className="w-8 h-8 text-blue-500 dark:text-blue-400" />
          </div>
          <div className="text-3xl font-bold text-blue-600 dark:text-blue-400 mb-1">
            {growthRate}%
          </div>
          <div className="w-full h-2 rounded-full bg-neutral-200 dark:bg-neutral-800 mt-2 mb-2">
            <div
              className="h-2 rounded-full bg-blue-600 dark:bg-blue-400 transition-all duration-500"
              style={{ width: `${growthRate}%` }}
            />
          </div>
          <div className="text-xs text-neutral-600 dark:text-neutral-400">
            {newUsers30d} new in 30 days
          </div>
        </div>
        ...
        <div className="bg-neutral-100 dark:bg-neutral-900 rounded-xl shadow p-6 flex flex-col transition-colors">
          <div className="flex items-center justify-between mb-2">
            ...
            <BuildingLibraryIcon className="w-8 h-8 text-green-600 dark:text-green-400" />
          </div>
        ...
        <div className="bg-purple-100 dark:bg-gradient-to-br dark:from-purple-800/40 dark:to-black 
          rounded-xl shadow p-6 flex flex-col transition-colors">
          <div className="flex items-center justify-between mb-2">
            <span className="text-lg font-semibold">Data Quality</span>
            <BookmarkIcon className="w-8 h-8 text-purple-500 dark:text-purple-300" />
          </div>
          <div className="text-3xl font-bold mb-1 text-purple-600 dark:text-fuchsia-400">100%</div>
          <div className="w-full h-2 rounded-full bg-neutral-200 dark:bg-neutral-800 mt-2 mb-2">
            <div
              className="h-2 rounded-full bg-purple-600 dark:bg-fuchsia-400 transition-all duration-500"
              style={{ width: `100%` }}
            />
          </div>
          <div className="text-xs text-neutral-600 dark:text-neutral-400">
      ...
      <div className="grid gap-5 grid-cols-1 md:grid-cols-2 max-w-7xl mx-auto">
        {/* Registration Trends */}
        <div className="bg-neutral-100 dark:bg-neutral-900 rounded-xl shadow p-6 transition-colors">
            ...
            {registrationTrends.map((trend, idx) => (
              <div key={idx} className="flex flex-col items-center flex-1">
                <div className="w-8 h-full flex flex-col justify-end">
                  <div
                    className="bg-blue-600 dark:bg-blue-500 rounded-t"
                    style={{
                      height: `${
                        (trend.count /
                          Math.max(...registrationTrends.map((t) => t.count || 1))) *
                          90 || 6
                      }px`,
                      minHeight: 6,
                    }}
